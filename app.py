        ...
        for page in pdf.pages:
            ...
            for raw in text.split("\n"):
                ...
                while True:
                    ...
                    line = ""  # den här raden är “konsumerad”, gå vidare
                ...
                if line and current:
                    ...
                    if am:
                        ...
                        tm = TRAILING_AMOUNT_RE.match(line)
                        if tm:
                            desc_part = tm.group(1).strip()
                            amt_str = tm.group(2)
                            try:
...
# Stegvis klassning
    ...
    if index < len(txs):
        ...
        df.to_excel(excel_buffer, index=False)
        excel_buffer.seek(0)
        st.download_button("📥 Ladda ner Excel-fil", data=excel_buffer, file_name="kategoriserade_utgifter.xlsx")        if "betalning mottagen" in desc.lower():
            current = None
            return
        # Datumjustering: flytta upp allt före start_date till start_date
        d = current["date"]
